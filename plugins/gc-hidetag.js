//Código desarollado por Semi, https://wa.me/50764880277)
let handler = async (m, { conn, text, participants }) => {
  ...
  if (!m.quoted && type) {
    if ((mime === 'imageMessage')) {
      ...
      conn.sendMessage(m.chat, { text: text ? text : "Pᴏʀɴʜᴜʙ: @BʏMunecaBot", mentions: participants.map(u => conn.decodeJid(u.id)) }, { quoted: m })
    }
  } else if (m.quoted) {
